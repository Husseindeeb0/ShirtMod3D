...
const Customizer = () => {
  ...
  useEffect(() => {
    const handleGlobalPointerDown = (event: MouseEvent | TouchEvent) => {
      ...
      if (!editorTabsRef.current.contains(target)) {
        setActiveEditorTab("");
      }
    };
  ...
  const handleActiveFilterTab = (tabName: keyof ActiveFilterTab) => {
    switch (tabName) {
      case "logoShirt":
          ...
          >
            {FilterTabs.map((tab) => (
              <Tab
                ...
                tab={tab}
                isFilterTab
                isActiveTab={activeFilterTab[tab.name as keyof ActiveFilterTab]}
                handleClick={() =>
                  handleActiveFilterTab(tab.name as keyof ActiveFilterTab)
                }
              />
            ))}
          </motion.div>
